...
module temple.func_string_gen2;
...
string build_function_head(string filter_ident) {
   ...
   ret ~= `void TempleFunc() {
	...
	{
		static if(__traits(compiles, __fp__.temple_filter(sink, thing))) {
			__fp__.temple_filter(sink, thing);
		}
		else static if(__traits(compiles, __fp__.temple_filter(thing)))
		{
			sink.put(__fp__.temple_filter(thing));
		}
		else {
	...
	{
		return TempleInputStream(delegate(ref TempleOutputStream s) {
			renderTempl!(__temple_str, Args)(s);
		});
	}
	`;
	}
	...
	foreach(index, immutable part; parts) {
		...
		with(FuncPart.Type) {
			...
			case StrLit:
				...
				func_str ~= `sink.put("` ~ part.value.replace("\n", "\\n").escapeQuotes() ~ "\");\n";
				break;
		}
	}
